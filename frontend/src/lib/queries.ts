...
export interface PlayerSplitsWithTeammate {
  ...
  with_teammate_min: number
  without_teammate_min: number
  with_teammate_fga: number
  without_teammate_fga: number
  with_teammate_fgm: number
  without_teammate_fgm: number
  with_teammate_fg_pct: number
  without_teammate_fg_pct: number
}

export interface MarketOddsDataPoint {
  timestamp: string
  odds: number
  market_id: number
}
 ...
 */
export async function getOddsHistory(marketId: number, hours = 24): Promise<MarketOddsDataPoint[]> {
  const result = await query(`
    SELECT
      recorded_at as timestamp,
      odds,
  ...
  return result.rows as MarketOddsDataPoint[]
}
...
export async function getHeadToHeadHistory(
  ...
  const result = await query(`
    SELECT
      ...
      (g.home_team_score + g.away_team_score) as total,
      CASE
        WHEN g.home_team_score > g.away_team_score THEN ht.abbreviation
    ...
    AND g.season = $3
    ORDER BY g.game_date DESC
  `, [teamAId, teamBId, currentSeason])

  return result.rows as H2HGame[]
}
...
/**
 * Get team performance splits with/without a specific player
 * Shows how the team performs when a player is active vs absent
 */
export async function getTeamSplitsWithPlayer(
  teamId: number,
  playerId: number
): Promise<TeamSplitsWithPlayer | null> {
  const currentSeason = await getCurrentSeason()
  ...
  const result = await query(`
    WITH team_games AS (
      SELECT
        ...
        CASE WHEN g.home_team_id = $1 THEN g.away_team_score ELSE g.home_team_score END as opp_score,
        (CASE WHEN g.home_team_id = $1 THEN g.home_team_score ELSE g.away_team_score END +
         CASE WHEN g.home_team_id = $1 THEN g.away_team_score ELSE g.home_team_score END) as total,
        EXISTS(
          SELECT 1 FROM player_game_stats pgs
          WHERE pgs.player_id = $2 AND pgs.game_id = g.game_id AND pgs.minutes > 0
        ) as player_active
      FROM games g
      WHERE (g.home_team_id = $1 OR g.away_team_id = $1)
        AND g.season = $3
    ...
    SELECT
      COUNT(CASE WHEN player_active THEN 1 END) as with_player_games,
      COUNT(CASE WHEN NOT player_active THEN 1 END) as without_player_games,
      ROUND(AVG(CASE WHEN player_active THEN team_score END), 1) as with_player_ppg,
      ROUND(AVG(CASE WHEN NOT player_active THEN team_score END), 1) as without_player_ppg,
      ROUND(AVG(CASE WHEN player_active THEN total END), 1) as with_player_total,
      ROUND(AVG(CASE WHEN NOT player_active THEN total END), 1) as without_player_total
    FROM team_games
  `, [teamId, playerId, currentSeason])

  if (!result.rows[0] || result.rows[0].with_player_games === 0) {
    return null
  }

  return {
    with_player_games: parseInt(String(result.rows[0].with_player_games)) || 0,
    without_player_games: parseInt(String(result.rows[0].without_player_games)) || 0,
    with_player_ppg: parseFloat(String(result.rows[0].with_player_ppg)) || 0,
    without_player_ppg: parseFloat(String(result.rows[0].without_player_ppg)) || 0,
    with_player_total: parseFloat(String(result.rows[0].with_player_total)) || 0,
    without_player_total: parseFloat(String(result.rows[0].without_player_total)) || 0
  }
}

/**
 * Get player performance splits with/without a specific teammate
 * Shows how a player performs when a teammate is active vs absent
 */
export async function getPlayerSplitsWithTeammate(
  playerId: number,
  teammateId: number
): Promise<PlayerSplitsWithTeammate | null> {
  const currentSeason = await getCurrentSeason()
  ...
  const result = await query(`
    WITH player_games AS (
      SELECT
        ...
        CASE WHEN pgs.minutes > 0 THEN
          (pgs.fg_attempted + 0.44 * pgs.ft_attempted + pgs.turnovers) /
          NULLIF((SELECT SUM(fg_attempted + 0.44 * ft_attempted + turnovers)
                  FROM player_game_stats pgs2
                  WHERE pgs2.game_id = g.game_id AND pgs2.team_id = pgs.team_id AND pgs2.minutes > 0), 0)
        ELSE 0 END as usage_pct,
        EXISTS(
          SELECT 1 FROM player_game_stats pgs_tm
          WHERE pgs_tm.player_id = $2 AND pgs_tm.game_id = g.game_id AND pgs_tm.minutes > 0
        ) as teammate_active
      FROM player_game_stats pgs
      JOIN games g ON pgs.game_id = g.game_id
      WHERE pgs.player_id = $1 AND g.season = $3 AND pgs.minutes > 0
    )
    SELECT
      COUNT(CASE WHEN teammate_active THEN 1 END) as with_teammate_games,
      COUNT(CASE WHEN NOT teammate_active THEN 1 END) as without_teammate_games,
      ROUND(AVG(CASE WHEN teammate_active THEN points END), 1) as with_teammate_pts,
      ROUND(AVG(CASE WHEN NOT teammate_active THEN points END), 1) as without_teammate_pts,
      ROUND(AVG(CASE WHEN teammate_active THEN usage_pct END) * 100, 1) as with_teammate_usage,
      ROUND(AVG(CASE WHEN NOT teammate_active THEN usage_pct END) * 100, 1) as without_teammate_usage,
      ROUND(AVG(CASE WHEN teammate_active THEN minutes END), 1) as with_teammate_min,
      ROUND(AVG(CASE WHEN NOT teammate_active THEN minutes END), 1) as without_teammate_min,
      ROUND(AVG(CASE WHEN teammate_active THEN fg_attempted END), 1) as with_teammate_fga,
      ROUND(AVG(CASE WHEN NOT teammate_active THEN fg_attempted END), 1) as without_teammate_fga,
      ROUND(AVG(CASE WHEN teammate_active THEN fg_made END), 1) as with_teammate_fgm,
      ROUND(AVG(CASE WHEN NOT teammate_active THEN fg_made END), 1) as without_teammate_fgm,
      ROUND(
        AVG(CASE WHEN teammate_active AND fg_attempted > 0 THEN ((fg_made::float / fg_attempted) * 100)::numeric END),
        1
      ) as with_teammate_fg_pct,
      ROUND(
        AVG(CASE WHEN NOT teammate_active AND fg_attempted > 0 THEN ((fg_made::float / fg_attempted) * 100)::numeric END),
        1
      ) as without_teammate_fg_pct
    FROM player_games
  `, [playerId, teammateId, currentSeason])

  if (!result.rows[0] || result.rows[0].with_teammate_games === 0) {
    return null
  }

  return {
    with_teammate_games: parseInt(String(result.rows[0].with_teammate_games)) || 0,
    without_teammate_games: parseInt(String(result.rows[0].without_teammate_games)) || 0,
    with_teammate_pts: parseFloat(String(result.rows[0].with_teammate_pts)) || 0,
    without_teammate_pts: parseFloat(String(result.rows[0].without_teammate_pts)) || 0,
    with_teammate_usage: parseFloat(String(result.rows[0].with_teammate_usage)) || 0,
    without_teammate_usage: parseFloat(String(result.rows[0].without_teammate_usage)) || 0,
    with_teammate_min: parseFloat(String(result.rows[0].with_teammate_min)) || 0,
    without_teammate_min: parseFloat(String(result.rows[0].without_teammate_min)) || 0,
    with_teammate_fga: parseFloat(String(result.rows[0].with_teammate_fga)) || 0,
    without_teammate_fga: parseFloat(String(result.rows[0].without_teammate_fga)) || 0,
    with_teammate_fgm: parseFloat(String(result.rows[0].with_teammate_fgm)) || 0,
    without_teammate_fgm: parseFloat(String(result.rows[0].without_teammate_fgm)) || 0,
    with_teammate_fg_pct: parseFloat(String(result.rows[0].with_teammate_fg_pct)) || 0,
    without_teammate_fg_pct: parseFloat(String(result.rows[0].without_teammate_fg_pct)) || 0
  }
}
...
export interface GameWithTeamRecords {
  game_id: string
  game_date: string
...
export async function getTeammatesWhenPlayerAbsent(
  ...
  const result = await query(`
    ...
    SELECT
      ...
      -- Minutes
      ROUND(AVG(CASE WHEN ts.player_played THEN ts.minutes END), 1) as with_min,
      ROUND(AVG(CASE WHEN NOT ts.player_played THEN ts.minutes END), 1) as without_min,
      ROUND(
        AVG(CASE WHEN NOT ts.player_played THEN ts.minutes END) -
        AVG(CASE WHEN ts.player_played THEN ts.minutes END), 1
      ) as min_boost,
      -- Starter percentage
      ROUND(
...
/**
 * Get H2H history between two teams (legacy - use getHeadToHeadHistory instead)
 * @deprecated Use getHeadToHeadHistory for complete data
 */
export async function getH2HHistory(team1Id: number, team2Id: number, limit = 10): Promise<H2HGame[]> {
  return getHeadToHeadHistory(team1Id, team2Id)
}
...
export interface TeamBettingLineHistory {
  game_date: string
  opponent: string
  location: 'HOME' | 'AWAY'
  team_ppg: number
...
/**
 * Get comprehensive betting line history for a specific team
 */
export async function getTeamBettingLineHistory(teamAbbr: string): Promise<TeamBettingLineHistory[]> {
  const currentSeason = await getCurrentSeason()
  ...
  const result = await query(`
    WITH team_games AS (
      SELECT
        g.game_id,
        g.game_date,
        CASE WHEN g.home_team_id = teams.team_id THEN 'HOME' ELSE 'AWAY' END as location,
        CASE WHEN g.home_team_id = teams.team_id THEN t_away.abbreviation ELSE t_home.abbreviation END as opponent,
        teams.team_id as team_id,
        CASE WHEN g.home_team_id = teams.team_id THEN g.away_team_id ELSE g.home_team_id END as opp_team_id,
        CASE WHEN g.home_team_id = teams.team_id THEN g.home_team_score ELSE g.away_team_score END as team_score,
        CASE WHEN g.home_team_id = teams.team_id THEN g.away_team_score ELSE g.home_team_score END as opp_score
      FROM games g
      JOIN teams ON teams.abbreviation = $1
        AND (g.home_team_id = teams.team_id OR g.away_team_id = teams.team_id)
      JOIN teams t_home ON g.home_team_id = t_home.team_id
      JOIN teams t_away ON g.away_team_id = t_away.team_id
      WHERE g.season = $2
        AND g.home_team_score IS NOT NULL
    ),
    team_stats_before_game AS (
      SELECT
        tg.game_id,
        tg.game_date,
        tg.opponent,
        tg.location,
        ROUND(AVG(CASE WHEN tgs.team_id = tg.team_id THEN tgs.points END), 1) as team_ppg,
        ROUND(AVG(CASE WHEN tgs.team_id = tg.opp_team_id THEN tgs.points END), 1) as opp_ppg,
        ROUND(AVG(CASE WHEN tgs.team_id = tg.team_id THEN tgs.pace END), 1) as team_pace,
        ROUND(AVG(CASE WHEN tgs.team_id = tg.opp_team_id THEN tgs.pace END), 1) as opp_pace,
        ROUND(AVG(CASE WHEN tgs.team_id = tg.team_id THEN tgs.offensive_rating END), 1) as team_ortg,
        ROUND(AVG(CASE WHEN tgs.team_id = tg.team_id THEN tgs.defensive_rating END), 1) as team_drtg,
        ROUND(AVG(CASE WHEN tgs.team_id = tg.opp_team_id THEN tgs.offensive_rating END), 1) as opp_ortg,
        ROUND(AVG(CASE WHEN tgs.team_id = tg.opp_team_id THEN tgs.defensive_rating END), 1) as opp_drtg,
        tg.team_score + tg.opp_score as actual_total
      FROM team_games tg
      LEFT JOIN games g_prior ON g_prior.season = $2
        AND g_prior.game_date < tg.game_date
        AND g_prior.home_team_score IS NOT NULL
        AND (g_prior.home_team_id IN (tg.team_id, tg.opp_team_id) OR g_prior.away_team_id IN (tg.team_id, tg.opp_team_id))
      LEFT JOIN team_game_stats tgs ON g_prior.game_id = tgs.game_id
        AND tgs.team_id IN (tg.team_id, tg.opp_team_id)
      GROUP BY tg.game_id, tg.game_date, tg.opponent, tg.location, tg.team_score, tg.opp_score
    ),
    betting_lines AS (
      SELECT
        be.game_id,
        -- Handle both storage formats:
        -- New format: line in market name (Full Game Game Total 226.5)
        -- Old format: line in handicap column (Full Game Total + handicap)
        COALESCE(
          AVG(SUBSTRING(bm.market_name FROM 'Game Total ([0-9.]+)')::numeric),
          AVG(bo.handicap)
        ) as total_line,
        MAX(CASE WHEN LOWER(bo.selection) = 'over' THEN bo.odds_decimal END) as over_odds,
        MAX(CASE WHEN LOWER(bo.selection) = 'under' THEN bo.odds_decimal END) as under_odds
      ...
      WHERE (bm.market_name = 'Full Game Total'
             OR bm.market_name LIKE 'Full Game Game Total%'
             OR bm.market_name LIKE 'Game Game Total%')
      GROUP BY be.game_id
    )
    SELECT
      ts.game_date,
      ...
      ts.team_ppg,
      ts.opp_ppg,
      ROUND(ts.team_ppg + ts.opp_ppg, 1) as implied_total,
      ts.team_pace,
      ts.opp_pace,
      ROUND((ts.team_pace + ts.opp_pace) / 2, 1) as avg_pace,
      ts.team_ortg,
      ts.team_drtg,
      ts.opp_ortg,
      ts.opp_drtg,
      ROUND(bl.total_line, 1) as book_line,
      ROUND(bl.total_line - (ts.team_ppg + ts.opp_ppg), 1) as line_adjustment,
      ts.actual_total,
      ROUND(ts.actual_total - bl.total_line, 1) as line_diff,
      CASE
        WHEN ts.actual_total > bl.total_line THEN 'OVER'
        WHEN ts.actual_total < bl.total_line THEN 'UNDER'
        ELSE 'PUSH'
      END as result,
      ROUND(bl.over_odds, 2) as over_odds,
      ROUND(bl.under_odds, 2) as under_odds
    FROM team_stats_before_game ts
    JOIN betting_lines bl ON ts.game_id = bl.game_id
  ...
  return result.rows.map(row => ({
    ...
    team_ppg: parseFloat(row.team_ppg) || 0,
    opp_ppg: parseFloat(row.opp_ppg) || 0,
    implied_total: parseFloat(row.implied_total) || 0,
    team_pace: parseFloat(row.team_pace) || 0,
    opp_pace: parseFloat(row.opp_pace) || 0,
    avg_pace: parseFloat(row.avg_pace) || 0,
    team_ortg: parseFloat(row.team_ortg) || 0,
    team_drtg: parseFloat(row.team_drtg) || 0,
    opp_ortg: parseFloat(row.opp_ortg) || 0,
    opp_drtg: parseFloat(row.opp_drtg) || 0,
    book_line: parseFloat(row.book_line) || 0,
    line_adjustment: parseFloat(row.line_adjustment) || 0,
    actual_total: parseInt(row.actual_total) || 0,
    line_diff: parseFloat(row.line_diff) || 0,
    result: row.result,
    over_odds: row.over_odds ? parseFloat(row.over_odds) : null,
    under_odds: row.under_odds ? parseFloat(row.under_odds) : null,
  }))
}
...
export interface OddsEvolutionGame {
  game_id: string
  game_date: string
  away_abbr: string
  home_abbr: string
...
export interface OddsMovementSummary {
  ...
  movement: {
    selection1: number  // percentage
    selection2: number  // percentage
  }
  updateCount: number
  dateRange: {
    start: string
...
/**
 * Get list of games with betting odds data
 */
export async function getGamesWithOdds(): Promise<OddsEvolutionGame[]> {
  const result = await query(`
    SELECT DISTINCT
      g.game_id,
      g.game_date,
      ...
      t1.full_name as away_team,
      t2.full_name as home_team,
      COUNT(DISTINCT bo.recorded_at) as update_count,
      MIN(bo.recorded_at) as first_odds,
      MAX(bo.recorded_at) as latest_odds
    FROM games g
    ...
    GROUP BY g.game_id, g.game_date, t1.abbreviation, t2.abbreviation,
             t1.full_name, t2.full_name
    HAVING COUNT(DISTINCT bo.recorded_at) > 0
    ORDER BY g.game_date DESC, latest_odds DESC
    LIMIT 50
  `)
  ...
  return result.rows.map(row => ({
    ...
    first_odds: row.first_odds,
    latest_odds: row.latest_odds,
  }))
}

/**
 * Get odds evolution for a specific game and market type
 */
export async function getOddsEvolution(
  gameId: string,
  marketType: 'totals' | 'moneyline' | 'spread',
  line?: number,
  spread?: number
): Promise<{ data: OddsDataPoint[], summary: OddsMovementSummary, teamNames: { home: string, away: string } }> {
  ...
  const teamNames = teamResult.rows[0] || { home_team: 'Home', away_team: 'Away' }

  // Determine market name pattern based on type
  let marketPattern: string
  if (marketType === 'totals') {
    marketPattern = '%Game Total%'
  } else if (marketType === 'moneyline') {
    marketPattern = 'Game Moneyline'
  } else {
    marketPattern = 'Game Spread%'
  }

  // Build WHERE clause dynamically based on whether line/spread filter is provided
  const params: any[] = [gameId, marketPattern]
  let whereClause = 'WHERE be.game_id = $1 AND bm.market_name LIKE $2'

  if (marketType === 'totals' && line !== undefined) {
    params.push(line)
    whereClause += ' AND bo.handicap = $3'
  } else if (marketType === 'spread' && spread !== undefined) {
    params.push(spread)
    // Filter by absolute value of handicap since we have both +7.0 and -7.0
    whereClause += ' AND ABS(bo.handicap) = $3'
  }
  ...
  const result = await query(`
    ...
    ${whereClause}
    ORDER BY bo.recorded_at ASC, bo.selection
  `, params)
  ...
  if (result.rows.length === 0) {
    return {
      data: [],
      summary: {
        openingOdds: { selection1: 0, selection2: 0 },
        currentOdds: { selection1: 0, selection2: 0 },
        movement: { selection1: 0, selection2: 0 },
        updateCount: 0,
        dateRange: { start: '', end: '' },
        selection1Label: '',
        selection2Label: '',
      },
      teamNames: { home: teamNames.home_team, away: teamNames.away_team },
    }
  }

  // Group by timestamp and pivot selections
  const dataMap = new Map<string, any>()

  result.rows.forEach((row: any) => {
    const timestamp = new Date(row.recorded_at).toISOString()

    if (!dataMap.has(timestamp)) {
      dataMap.set(timestamp, {
        timestamp: new Date(row.recorded_at).toLocaleString('en-US', {
          month: 'short',
          day: 'numeric',
          hour: '2-digit',
          minute: '2-digit',
    ...
    if (marketType === 'totals') {
      const line = row.market_name.match(/([0-9.]+)/)?.[1]
      if (line) dataPoint.line = parseFloat(line)

      if (row.selection.toLowerCase().includes('over')) {
        dataPoint.over = parseFloat(row.odds_decimal)
      } else if (row.selection.toLowerCase().includes('under')) {
        dataPoint.under = parseFloat(row.odds_decimal)
      }
    } else if (marketType === 'moneyline') {
      dataPoint.homeTeam = teamNames.home_team
      dataPoint.awayTeam = teamNames.away_team

      if (row.selection === teamNames.home_team) {
        dataPoint.home = parseFloat(row.odds_decimal)
      } else if (row.selection === teamNames.away_team) {
        dataPoint.away = parseFloat(row.odds_decimal)
      }
    } else if (marketType === 'spread') {
      if (row.handicap !== null) {
        dataPoint.handicap = parseFloat(row.handicap)
      }

      // Selection includes spread notation like "Orlando Magic (-5.5)" so we use includes() instead of exact match
      if (row.selection.includes(teamNames.home_team)) {
        dataPoint.homeSpread = parseFloat(row.odds_decimal)
      } else if (row.selection.includes(teamNames.away_team)) {
        dataPoint.awaySpread = parseFloat(row.odds_decimal)
      }
    }
  })
  ...
  if (marketType === 'totals') {
    selection1Label = `Over ${firstPoint.line || ''}`
    selection2Label = `Under ${firstPoint.line || ''}`
    opening1 = firstPoint.over || 0
    opening2 = firstPoint.under || 0
    current1 = lastPoint.over || 0
    current2 = lastPoint.under || 0
  } else if (marketType === 'moneyline') {
    selection1Label = teamNames.home_team
    selection2Label = teamNames.away_team
    opening1 = firstPoint.home || 0
    opening2 = firstPoint.away || 0
    current1 = lastPoint.home || 0
    current2 = lastPoint.away || 0
  } else {
    selection1Label = `${teamNames.home_team} ${lastPoint.handicap > 0 ? '+' : ''}${lastPoint.handicap || ''}`
    selection2Label = teamNames.away_team
    opening1 = firstPoint.homeSpread || 0
    opening2 = firstPoint.awaySpread || 0
    current1 = lastPoint.homeSpread || 0
    current2 = lastPoint.awaySpread || 0
  }

  const movement1 = opening1 !== 0 ? ((current1 - opening1) / opening1) * 100 : 0
  const movement2 = opening2 !== 0 ? ((current2 - opening2) / opening2) * 100 : 0

  const summary: OddsMovementSummary = {
    openingOdds: { selection1: opening1, selection2: opening2 },
    currentOdds: { selection1: current1, selection2: current2 },
    movement: { selection1: movement1, selection2: movement2 },
...
export async function getPlayerPropsOddsEvolution(
  gameId: string,
  playerName: string,
  propType: string,
  line: number
): Promise<{ data: OddsDataPoint[], summary: OddsMovementSummary, playerInfo: { playerName: string, propType: string, line: number } }> {
  // Fetch player props odds evolution
  const result = await query(`
    SELECT
      recorded_at,
      over_odds_decimal,
    ...
    ORDER BY recorded_at ASC
  `, [gameId, playerName, propType, line])
  ...
  if (result.rows.length === 0) {
    return {
      ...
      playerInfo: { playerName, propType, line },
    }
  }

  // Transform data for charting
  const dataMap = new Map<string, OddsDataPoint>()

  result.rows.forEach((row: any) => {
    const recordedAt = new Date(row.recorded_at)
    const timestamp = recordedAt.toLocaleString('en-US', {
      month: 'short',
      day: 'numeric',
      hour: '2-digit',
      minute: '2-digit',
    ...
    // Add line/handicap
    if (row.handicap !== null) {
      dataPoint.line = parseFloat(row.handicap)
    }

    // Add odds
    if (row.over_odds_decimal !== null) {
      dataPoint.over = parseFloat(row.over_odds_decimal)
    }
    if (row.under_odds_decimal !== null) {
      dataPoint.under = parseFloat(row.under_odds_decimal)
    }
  })
  ...
  const opening1 = firstPoint.over || 0
  const opening2 = firstPoint.under || 0
  const current1 = lastPoint.over || 0
  const current2 = lastPoint.under || 0

  const movement1 = opening1 !== 0 ? ((current1 - opening1) / opening1) * 100 : 0
  const movement2 = opening2 !== 0 ? ((current2 - opening2) / opening2) * 100 : 0

  // Format prop type for display
  const propTypeDisplay = propType.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase())
  ...
  const summary: OddsMovementSummary = {
    ...
    selection1Label: `${playerName} Over ${line} ${propTypeDisplay}`,
    selection2Label: `${playerName} Under ${line} ${propTypeDisplay}`,
  }
...
// ============================================
// LA UNE - GAME TOP PERFORMERS
// ============================================

export interface GameTopPerformer {
  player_id: number
  full_name: string
  team_id: number
  team_abbr: string
...
/**
 * Get top performers for a specific game (top 3 scorers per team)
 */
export async function getGameTopPerformers(gameId: string): Promise<{
  home: GameTopPerformer[]
  away: GameTopPerformer[]
}> {
  const result = await query(`
    WITH game_info AS (
    ...
    ranked_players AS (
      SELECT
        ...
        pgs.assists,
        pgs.minutes,
        CASE WHEN pgs.team_id = gi.home_team_id THEN 'home' ELSE 'away' END as side,
        ROW_NUMBER() OVER (
          PARTITION BY CASE WHEN pgs.team_id = gi.home_team_id THEN 'home' ELSE 'away' END
          ORDER BY pgs.points DESC
        ) as rank
      FROM player_game_stats pgs
      JOIN players p ON pgs.player_id = p.player_id
      JOIN teams t ON pgs.team_id = t.team_id
      JOIN game_info gi ON pgs.team_id IN (gi.home_team_id, gi.away_team_id)
      WHERE pgs.game_id = $1 AND pgs.minutes > 0
    )
    SELECT * FROM ranked_players WHERE rank <= 3
  `, [gameId])

  const home: GameTopPerformer[] = []
  const away: GameTopPerformer[] = []

  for (const row of result.rows) {
    const performer: GameTopPerformer = {
      player_id: row.player_id,
      full_name: row.full_name,
      team_id: row.team_id,
...
/**
 * Get full box scores for all players in a game, grouped by team
 */
export async function getGameBoxScores(gameId: string): Promise<{
  home: { team_abbr: string; team_name: string; players: PlayerBoxScore[] }
  away: { team_abbr: string; team_name: string; players: PlayerBoxScore[] }
} | null> {
  const result = await query(`
    WITH game_info AS (
      SELECT home_team_id, away_team_id
    ...
    SELECT
      pgs.player_id,
      p.full_name,
      pgs.start_position as position,
      pgs.is_starter,
      pgs.minutes,
      pgs.points,
      pgs.rebounds,
    ...
    WHERE pgs.game_id = $1
    ORDER BY
      CASE WHEN pgs.team_id = gi.home_team_id THEN 0 ELSE 1 END,
      pgs.is_starter DESC,
      pgs.minutes DESC
  `, [gameId])
  ...
  const home: { team_abbr: string; team_name: string; players: PlayerBoxScore[] } = {
    team_abbr: '',
    team_name: '',
    players: []
  }
  const away: { team_abbr: string; team_name: string; players: PlayerBoxScore[] } = {
    team_abbr: '',
    team_name: '',
    players: []
  }
  ...
  for (const row of result.rows) {
    const player: PlayerBoxScore = {
      ...
      personal_fouls: row.personal_fouls || 0,
      fg_made: row.fg_made || 0,
      fg_attempted: row.fg_attempted || 0,
      fg3_made: row.fg3_made || 0,
      fg3_attempted: row.fg3_attempted || 0,
      ft_made: row.ft_made || 0,
      ft_attempted: row.ft_attempted || 0,
    ...
    if (row.side === 'home') {
      if (!home.team_abbr) {
        home.team_abbr = row.team_abbr
        home.team_name = row.team_name
      }
      home.players.push(player)
    } else {
      if (!away.team_abbr) {
        away.team_abbr = row.team_abbr
        away.team_name = row.team_name
      }
...
export interface BettingRecap {
  // Moneyline
  moneyline: {
    homeOdds: number        // decimal odds (e.g., 1.45)
    awayOdds: number        // decimal odds (e.g., 2.85)
    favorite: 'home' | 'away'
    favoriteConfirmed: boolean
    upset: boolean
    marginPoints: number    // home_score - away_score
  } | null

  // Spread
  spread: {
    line: number            // spread from home perspective (negative = home favored)
    homeOdds: number        // decimal odds
    awayOdds: number        // decimal odds
    result: 'COVER' | 'LOSS' | 'PUSH'
    coverMargin: number     // actual_margin - spread_line
  } | null
  ...
  gameTotal: {
    ...
    actualTotal: number     // combined final score
    result: 'OVER' | 'UNDER' | 'PUSH'
 ...
 * Positive: (American / 100) + 1 → e.g., +370 = 4.70
 * Negative: (100 / |American|) + 1 → e.g., -476 = 1.21
 */
...
/**
 * Get betting recap for a completed game
 * Returns moneyline, spread, and totals results with analysis data
 */
export async function getGameBettingRecap(gameId: string): Promise<BettingRecap | null> {
  // Fetch game info and betting lines
  const result = await query(`
    SELECT
      g.game_id,
      g.game_date,
  ...
  const row = result.rows[0]
  const homeScore = row.home_team_score
  const awayScore = row.away_team_score
  ...
  // Build moneyline section
  let moneyline: BettingRecap['moneyline'] = null
  if (row.home_moneyline !== null && row.away_moneyline !== null) {
    const homeOdds = americanToDecimal(row.home_moneyline)
    const awayOdds = americanToDecimal(row.away_moneyline)
    const favorite: 'home' | 'away' = homeOdds < awayOdds ? 'home' : 'away'
    const winner: 'home' | 'away' = actualMargin > 0 ? 'home' : 'away'
    const favoriteConfirmed = favorite === winner
    const upset = !favoriteConfirmed
    ...
    moneyline = {
      ...
      favoriteConfirmed,
      upset,
      marginPoints: actualMargin,
    }
  }

  // Build spread section
  let spread: BettingRecap['spread'] = null
  if (row.spread !== null) {
    const spreadLine = parseFloat(row.spread) // negative = home favored
    const coverMargin = actualMargin - spreadLine

    let spreadResult: 'COVER' | 'LOSS' | 'PUSH'
    if (coverMargin > 0) {
      spreadResult = 'COVER' // Home covered
    } else if (coverMargin < 0) {
      spreadResult = 'LOSS' // Home didn't cover
    } else {
      spreadResult = 'PUSH'
    }

    // Default odds if not available (typically -110 = 1.91)
    const homeSpreadOdds = row.home_spread_odds ? americanToDecimal(row.home_spread_odds) : 1.91
    const awaySpreadOdds = row.away_spread_odds ? americanToDecimal(row.away_spread_odds) : 1.91

    spread = {
      line: spreadLine,
      homeOdds: parseFloat(homeSpreadOdds.toFixed(2)),
      awayOdds: parseFloat(awaySpreadOdds.toFixed(2)),
      result: spreadResult,
      coverMargin,
    }
  }

  // Build game total section
  let gameTotal: BettingRecap['gameTotal'] = null
  if (row.total !== null) {
    const totalLine = parseFloat(row.total)
    const margin = actualTotal - totalLine

    let totalResult: 'OVER' | 'UNDER' | 'PUSH'
    if (margin > 0) {
      totalResult = 'OVER'
    } else if (margin < 0) {
      totalResult = 'UNDER'
    ...
    const overOdds = row.over_odds ? americanToDecimal(row.over_odds) : 1.91
    const underOdds = row.under_odds ? americanToDecimal(row.under_odds) : 1.91

    gameTotal = {
      line: totalLine,
      overOdds: parseFloat(overOdds.toFixed(2)),
      underOdds: parseFloat(underOdds.toFixed(2)),
      actualTotal,
      result: totalResult,
      margin,
    }
