...
type StatType = 'pts' | 'reb' | 'ast' | 'stl' | 'blk' | '3pm' | 'fg' | 'min' | 'fga' | 'fgm'
...
const statConfig: Record<StatType, { label: string; shortLabel: string }> = {
  ...
  min: { label: 'Minutes', shortLabel: 'MIN' },
  fga: { label: 'Tentatives', shortLabel: 'FGA' },
  fgm: { label: 'Réussis', shortLabel: 'FGM' },
}
...
export function AbsenceCascadeView({ absentPlayer, teammates }: AbsenceCascadeViewProps) {
  ...
  const getStatValues = (teammate: TeammatePerformanceSplit) => {
    switch (activeStat) {
      ...
      case 'min':
        return {
          with: toNum(teammate.with_min),
          without: toNum(teammate.without_min),
          boost: toNum(teammate.min_boost),
        }
      case 'fga':
        return {
          with: toNum(teammate.with_fga),
          without: toNum(teammate.without_fga),
          boost: toNum(teammate.without_fga) - toNum(teammate.with_fga),
        }
      case 'fgm':
        return {
          with: toNum(teammate.with_fgm),
  ...
  const getBoostColor = (boost: number, stat: StatType) => {
    const threshold =
      stat === 'blk' || stat === 'stl' || stat === '3pm' ? 1 :
      stat === 'fg' || stat === 'fga' || stat === 'fgm' ? 2 :
      stat === 'min' ? 4 :
      3
    const smallThreshold =
      stat === 'blk' || stat === 'stl' || stat === '3pm' ? 0.5 :
      stat === 'fg' || stat === 'fga' || stat === 'fgm' ? 1 :
      stat === 'min' ? 2 :
      1.5
