...
import {
  getCurrentSeason,
  getPlayerStatsWithRankings,
  getPlayerGamelogs,
  getTeammatesWhenPlayerAbsent,
...
import { PlayerInsight, PlayerAnalysis } from '@/components/teams'
...
async function getPlayerData(playerId: number) {
  ...
  // Extract player info for second round of queries
  const playerInfo = playerInfoResult.rows[0]
  const topTeammate = teammatesWhenAbsent && teammatesWhenAbsent.length > 0
    ? teammatesWhenAbsent[0]
    : null

  // Fetch absence impact data (requires team_id and teammate_id from first round)
  const [teamSplits, playerTeammateSplits] = await Promise.all([
    playerInfo?.team_id
      ? getTeamSplitsWithPlayer(playerInfo.team_id, playerId)
      : Promise.resolve(null),
    topTeammate?.player_id
      ? getPlayerSplitsWithTeammate(playerId, topTeammate.player_id)
      : Promise.resolve(null)
  ])
...
export default async function PlayerDetailPage({ params }: PageProps) {
  ...
  const { playerInfo, seasonStats, recentGames, advancedStats, positionRanking, teammates, playerProps, propsAvg, seasonGames, playerStatsWithRankings, playerGamelogs, teammatesWhenAbsent, teamSplits, playerTeammateSplits, topTeammate, currentSeason } = data
    ...
    <AppLayout>
      ...
      <section className="pt-8 pb-16 px-4 sm:px-8 lg:px-12">
        <div className="w-full">
          ...
          {/* Season Overview Insight (after hero stats) */}
          {seasonStats && stats.games > 0 && (
            <PlayerInsight
              playerId={playerIdNum}
              playerName={playerInfo.full_name}
              componentType="seasonOverview"
              contextData={{
                ppg: stats.ppg,
                rpg: stats.rpg,
                apg: stats.apg,
                ...
                team: playerInfo.team_abbreviation,
                ...(ranking ? { ppgRank: ranking.ppgRank, rpgRank: ranking.rpgRank, apgRank: ranking.apgRank, totalAtPosition: ranking.total } : {}),
              }}
              className="mt-6"
            />
          )}
      ...
      {/* SECTION 05.5: ABSENCE IMPACT */}
      {((teammatesWhenAbsent && teammatesWhenAbsent.length > 0 && seasonGames) || teamSplits || playerTeammateSplits) && (
        <section className="py-20 px-4 sm:px-8 lg:px-12">
          <div className="w-full">
            {/* Section identifier */}
            <div className="mb-8 flex items-center gap-4">
            ...
            {/* Teammate Performance Cascade (only if enough absence data) */}
            ...
            {/* Team Performance With/Without This Player */}
            {teamSplits && (
              <div className={teammatesWhenAbsent && teammatesWhenAbsent.length > 0 ? "mt-12" : ""}>
                <h3 className="text-xl font-bold text-white mb-4">
                  {playerInfo.team_abbreviation} Performance With/Without {playerInfo.full_name}
                </h3>
                <TeamPerformanceWithoutPlayer
                  playerName={playerInfo.full_name}
                  teamName={playerInfo.team_abbreviation || 'TEAM'}
                  splits={teamSplits}
                />
              </div>
            )}

            {/* Player Performance With/Without Top Teammate */}
            {playerTeammateSplits && topTeammate && (
              <div className="mt-12">
                <h3 className="text-xl font-bold text-white mb-4">
                  {playerInfo.full_name} Performance With/Without {topTeammate.teammate_name}
                </h3>
                <PlayerPerformanceWithoutTeammate
                  playerName={playerInfo.full_name}
                  teammateName={topTeammate.teammate_name}
                  splits={playerTeammateSplits}
                />
              </div>
            )}
          </div>
      ...
      {playerProps && playerProps.length > 0 && (
        <section className="py-20 px-4 sm:px-8 lg:px-12">
          <div className="w-full">
            ...
            {/* Props Analysis Insight */}
            <PlayerInsight
              playerId={playerIdNum}
              playerName={playerInfo.full_name}
              componentType="propsAnalysis"
              contextData={{
                props: playerProps.map(p => ({ type: p.prop_type, line: parseFloat(p.line) })),
                propsAvg: propsAvg.map(p => ({ type: p.prop_type, avg: parseFloat(p.avg_value) })),
                last10Ppg: last10Stats?.ppg,
              }}
              className="mt-4"
            />
          </div>
        ...
        <section className="py-20 px-4 sm:px-8 lg:px-12">
          <div className="w-full">
            ...
            <PlayerInsight
              ...
              contextData={{
                last10Ppg: last10Stats.ppg,
                last10Wins: last10Stats.wins,
                last10Losses: last10Stats.losses,
                last10High: last10Stats.highGame,
                last10Low: last10Stats.lowGame,
                seasonPpg: stats.ppg,
              }}
              className="mt-4"
            />
      ...
      {advancedStats && (advanced.tsPct > 0 || advanced.efgPct > 0) && (
        <section className="py-20 px-4 sm:px-8 lg:px-12">
          <div className="w-full">
            ...
            {/* Advanced Profile Insight */}
            <PlayerInsight
              playerId={playerIdNum}
              playerName={playerInfo.full_name}
              componentType="advancedProfile"
              contextData={{
                tsPct: advanced.tsPct,
                efgPct: advanced.efgPct,
                usage: advanced.usage,
      ...
      {/* SECTION 09: PLAYER ANALYSIS (RAG Narrative) */}
      <section className="py-20 px-4 sm:px-8 lg:px-12">
        <div className="w-full">
          <div className="mb-8 flex items-center gap-4">
            <span className="text-sm font-medium text-zinc-500 tracking-widest font-mono">09</span>
            <div className="h-px flex-1 bg-zinc-800" />
          </div>
          ...
          <PlayerAnalysis
            playerId={playerIdNum}
            playerName={playerInfo.full_name}
            teamAbbreviation={playerInfo.team_abbreviation || 'N/A'}
          />
        </div>
      </section>
